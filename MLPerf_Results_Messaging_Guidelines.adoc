...
== Performance optimizations must not be used to disparage or diminish* valid MLPerf results

All valid MLPerf submissions satisfy the same accuracy or quality requirements for that benchmark, regardless of the performance optimizations used. Submitters disclose details of their optimizations, including but not limited to lowest numerical precision (e.g., FP8, FP4), parallelism strategies, communication overlaps, or compiler techniques, to increase transparency and provide more insights. Whether or not such disclosure is made, the following rules apply:

  - Submitters may not imply or state that a valid result is inferior, does not sufficiently converge, or is otherwise less valid due to the performance optimizations used.

pass:[*]disparage or diminish means to negatively position another submitter's results 

**Non-Compliant Example**:

____
LesserSmartAI Corp will not compare against BF16 results submitted by SmartAI Corp because BF16 recipes converge poorly.
____

This violates these Guidelines because it implies that valid MLPerf results achieved using BF16 precision are less legitimate than other higher results although both meet the same required accuracy target and are equally valid MLPerf submissions.
